...
class CMlp(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
'''
class AttnCBlock_side(nn.Module):
    ...
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, dropout=None, side_dim=384, T=8, patch_num=49):
        ...
        for i in range(layers):
            ...
            if i not in self.drop_layer:
                ...
                #if i == 0:
                #    self.side_c5_transformer.append(AttnCBlock_side(self.side_dim, int(self.side_dim * self.temporal_ratio), kernel_size=1, T=self.T))
        ...
        #self.side_c5_transformer = nn.ModuleList(self.side_c5_transformer)
        #self.conv_m = conv_3xnxn_std(self.side_dim, self.side_dim, kernel_size=3, stride=1)
        #self.bn3d_m = nn.BatchNorm3d(self.side_dim)
